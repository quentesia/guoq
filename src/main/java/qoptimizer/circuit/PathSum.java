...
public class PathSum {
    ...
    public static void cx(Circuit circuit, String control, String target) {
        if (!circuit.hasQubit(control)) {
            circuit.addQubit(control);
        }
        if (!circuit.hasQubit(target)) {
            circuit.addQubit(target);
        }
    ...
    public static void rxx(Circuit circuit, String qubit1, String qubit2, Expr angle) {
        if (!circuit.hasQubit(qubit1)) {
            circuit.addQubit(qubit1);
        }
        if (!circuit.hasQubit(qubit2)) {
