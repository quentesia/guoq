...
import qoptimizer.circuit.ConstrainedCircuit;
import qoptimizer.circuit.EquivalenceClass;
        ...
        for (int i = 1; i <= size; i++) {
            ...
            if (i == 1) {
                ...
                for (ConstrainedCircuit c : previousReps.values()) {
                    ...
                    // apply other gates
                    for (String gate : this.gates) {
                        if ((gate.equals("cx") || gate.equals("cz")) && c.getCircuit().hasSymb()) {
                            continue;
                        }
                        for (int q = 0; q <= Math.min(c.getCircuit().getQubits().size(), numQubits - 1); q++) {
                            if (c.getCircuit().hasSymb() && q >= MAX_QUBITS_SYMB) {
                                continue;
                            }
...
//    System.out.println(String.format("total: %s, make circuit smaller: %s, have symb: %s", total, makeCircuitSmaller, haveSymb));
    ...
    private List<Circuit> applyGate(Circuit c, String gate, int qubit, int numQubits) {
        ...
        map.put("x", new Command() {
            @Override
            public void runOneQubit(Circuit circ, String q) {
                PathSum.x(circ, q);
            }
        ...
        map.put("rz", new Command() {
            @Override
            public void runOneQubitOneParam(Circuit circ, String q, Expr angle) {
                PathSum.rz(circ, q, angle);
            }
        ...
        map.put("cx", new Command() {
            @Override
            public void runTwoQubit(Circuit circ, String q1, String q2) {
                PathSum.cx(circ, q1, q2);
            }
        ...
        map.put("symb", new Command() {
            @Override
            public void runSymb(Circuit circ, int numQubits) {
                PathSum.symb(circ, numQubits);
            }
        ...
        map.put("u2", new Command() {
            @Override
            public void runOneQubitTwoParam(Circuit circ, String q, Expr angle1, Expr angle2) {
                PathSum.u2(circ, q, angle1, angle2);
            }
        ...
        map.put("u3", new Command() {
            @Override
            public void runOneQubitThreeParam(Circuit circ, String q, Expr angle1, Expr angle2, Expr angle3) {
                PathSum.u3(circ, q, angle1, angle2, angle3);
            }
        ...
        map.put("rxx", new Command() {
            @Override
            public void runTwoQubitOneParam(Circuit circ, String q1, String q2, Expr angle) {
                PathSum.rxx(circ, q1, q2, angle);
            }
        ...
        map.put("ms", new Command() {
            @Override
            public void runTwoQubitTwoParam(Circuit circ, String q1, String q2, Expr angle1, Expr angle2) {
                PathSum.ms(circ, q1, q2, angle1, angle2);
            }
        ...
        map.put("sx", new Command() {
            @Override
            public void runOneQubit(Circuit circ, String q1) {
                PathSum.sx(circ, q1);
            }
        ...
        switch (gate) {
            ...
            case "t": {
                Circuit deepCopy = copy(c);
                map.get("rz").runOneQubitOneParam(deepCopy, name, new BinOp(Op.DIV, new Symbol("pi"), new Real(4)));
                deepCopy.getQasm().remove(deepCopy.getQasm().size() - 1);
                deepCopy.getQasm().add(String.format("t %s", name));
                circuitsAfterApply.add(deepCopy);
                return circuitsAfterApply;
            }
            case "s": {
                Circuit deepCopy = copy(c);
                map.get("rz").runOneQubitOneParam(deepCopy, name, new BinOp(Op.DIV, new Symbol("pi"), new Real(2)));
                deepCopy.getQasm().remove(deepCopy.getQasm().size() - 1);
                deepCopy.getQasm().add(String.format("s %s", name));
                circuitsAfterApply.add(deepCopy);
                return circuitsAfterApply;
            }
            case "tdg": {
                Circuit deepCopy = copy(c);
                map.get("rz").runOneQubitOneParam(deepCopy, name, new UnOp(Op.MINUS, new BinOp(Op.DIV, new Symbol("pi"), new Real(4))));
                deepCopy.getQasm().remove(deepCopy.getQasm().size() - 1);
                deepCopy.getQasm().add(String.format("tdg %s", name));
                circuitsAfterApply.add(deepCopy);
                return circuitsAfterApply;
            }
            case "sdg": {
                Circuit deepCopy = copy(c);
                map.get("rz").runOneQubitOneParam(deepCopy, name, new UnOp(Op.MINUS, new BinOp(Op.DIV, new Symbol("pi"), new Real(2))));
                deepCopy.getQasm().remove(deepCopy.getQasm().size() - 1);
                deepCopy.getQasm().add(String.format("sdg %s", name));
                circuitsAfterApply.add(deepCopy);
                return circuitsAfterApply;
            }
            case "rx1": {
            ...
            default:
                throw new RuntimeException("unimplemented gate");
        }
    }
...
//    String[] gates = {"gpi", "gpi2", "vz", "ms"};
        ...
        Option gatesetO = new Option("g", "gateSet", true, "gate set (options: nam, ibm, ibmnew, rigetti, ion, cliffordt)");
        ...
        try {
            ...
            switch (gateset) {
                ...
                case "cliffordt": {
                    String[] gates = {"h", "cx", "t", "tdg", "s", "sdg", "x"};
                    Expr[] symbAngles = {};
                    enumerator = new Synthesizer(gates, maxQubits, rand, symbAngles);
                    break;
                }
...
//      System.out.println("enumerate time (s): " + ((time2-time1)/1000));
            ...
            formatter.printHelp("QUESO Rule qoptimizer.Synthesizer", options);
