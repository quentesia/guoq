...
import qoptimizer.circuit.Concrete;
...
//    System.out.println(v2);
    ...
    private Circuit subtractCircuits(Circuit c1, Circuit c2) {
        for (String qubit : c1.getUsedQubits()) {
            if (!c2.hasQubit(qubit)) {
                c2.addQubit(qubit);
            }
        }
        for (String qubit : c2.getUsedQubits()) {
            if (!c1.hasQubit(qubit)) {
                c1.addQubit(qubit);
            }
        }
...
//      System.out.println(eval_phi);
    ...
    private <T> boolean containsTerm(Expr e, Class<T> term) {
        ...
        switch (e) {
            case UnOp uo:
                return containsTerm(uo.getE(), term);
            case BinOp bo:
                return containsTerm(bo.getE1(), term) || containsTerm(bo.getE2(), term);
            default:
                return false;
        }
    }
    ...
    private boolean containsUnop(Expr e, Op op) {
        ...
        switch (e) {
            case UnOp uo:
                return containsUnop(uo.getE(), op);
            case BinOp bo:
                return containsUnop(bo.getE1(), op) || containsUnop(bo.getE2(), op);
            default:
                return false;
        }
    }
    ...
    private Expr simplifyUnop(UnOp uo, Map<String, Integer> qubitMap, Map<String, Expr> funMap) {
        ...
        switch (uo.getOp()) {
            ...
            default:
                return new UnOp(uo.getOp(), simpE);
        }
    }
    ...
    private Expr simplifyBinop(BinOp bo, Map<String, Integer> qubitMap, Map<String, Expr> funMap) {
        ...
        switch (bo.getOp()) {
            ...
            default:
                return new BinOp(bo.getOp(), simpE1, simpE2);
        }
    }
    ...
    private Expr simplifyExpr(Expr e, Map<String, Integer> qubitMap, Map<String, Expr> funMap) {
        switch (e) {
            ...
            case Symbol s:
                if (s.getSymbol().equals("phi")) {
                    return new Symbol("phi" + qubitMap);
                } else {
                    return s;
                }
            case Bool b:
                return b.isBool() ? new Real(1) : new Real(0);
            case Var v:
                return new Real(qubitMap.get(v.getId()));
            case Fun f:
                return new Real(assertZeroOrOne(evalFun(f, qubitMap, new HashMap<>(), funMap)));
            case UnOp uo:
                return simplifyUnop(uo, qubitMap, funMap);
            case BinOp bo:
                return simplifyBinop(bo, qubitMap, funMap);
            default:
                return e;
        }
    }
    ...
    private ComplexRat evaluateExpr(Expr e, Map<String, ComplexRat> samples, ArrayList<ComplexRat> multOtherSide) {
        switch (e) {
            ...
            case Bool b:
                return b.isBool() ? ComplexRat.ONE : ComplexRat.ZERO;
            case Symbol v:
                if (samples.containsKey(v.getSymbol())) {
            ...
            case UnOp uo:
                return evaluateUnop(uo, samples, multOtherSide);
            case BinOp bo:
                return evaluateBinop(bo, samples, multOtherSide);
            case Real r:
                return new ComplexRat(new BigFraction((int) r.getNumber()), BigFraction.ZERO);
            default:
                System.out.println(e);
                throw new RuntimeException("evaluateExpr");
        }
    }
    ...
    private ComplexRat evaluateUnop(UnOp uo, Map<String, ComplexRat> samples, ArrayList<ComplexRat> multOtherSide) {
        switch (uo.getOp()) {
            case EXP:
                if (containsTerm(uo, Symbol.class)) {
...
//          }
            ...
            default:
                throw new RuntimeException("unimplemented unop evaluateUnop: " + uo.getOp());
        }
    }
    ...
    public Complex evalExpr(Expr e,
        ...
        switch (e) {
            case Real r:
                return new Complex(r.getNumber());
            case Bool b:
                return b.isBool() ? Complex.ONE : Complex.ZERO;
            case Var v:
                return new Complex(qubitMap.get(v.getId()));
            case Symbol s:
                return evalSymbol(s, qubitMap, symbolMap, funMap);
            case Fun f:
                return evalFun(f, qubitMap, symbolMap, funMap);
            case UnOp uo:
                return evalUnOp(uo, qubitMap, symbolMap, funMap);
            case BinOp bo:
                return evalBinOp(bo, qubitMap, symbolMap, funMap);
            case Constant c:
                if (c.getId().equals("i")) {
                    return Complex.I;
    ...
    private Complex evalUnOp(UnOp uo,
        ...
        switch (uo.getOp()) {
            case EXP:
                return Complex.I.multiply(v).exp();
            case SQRT:
                return v.sqrt();
            case MINUS:
                return v.negate();
...
//    qoptimizer.circuit.Circuit c2 = new qoptimizer.circuit.Circuit(qubits2, pathSum2, new ArrayList<>());
//
...
//    rand.setSeed(54);
//    qoptimizer.Verifier verifier = new qoptimizer.Verifier(rand, 2);
